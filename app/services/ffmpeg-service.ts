...
function logProgress(percent: number): void {
  ...
  if (!inNewProcess && validPercent < highestSeenPercentage - 5 && highestSeenPercentage > 10) {
  ...
  if (Math.abs(validPercent - lastReportedPercent) < 2 && validPercent !== 0 && validPercent !== 100) {
    return;
  }
  
  // Create progress bar
  const progressBar = '█'.repeat(Math.floor(validPercent / 5)) + '░'.repeat(20 - Math.floor(validPercent / 5));
  console.log(`${getTimestamp()} 🔄 Processing: [${progressBar}] ${validPercent}%`);
...
// Create a video from an image and audio file with slow zoom effect
export async function createVideo(
  ...
  try {
    console.log(`Creating video with image ${imagePath} and audio ${audioPath}`);
    
    // Ensure the processing directory exists
    const processingDir = path.join(config.paths.tempDir, config.paths.processingDir);
    ...
    return new Promise((resolve, reject) => {
      ff()
        ...
        .complexFilter([
          ...
          `[0:v]scale=1920:1080,zoompan=z='min(zoom+0.0001,1.1)':d=${duration*30}:s=1920x1080:fps=30[v]`
        ])
        .outputOptions([
          '-map', '[v]',     // Use the video after zoom effect
        ...
        .on('progress', (progress: any) => {
          ...
          logStream.write(`${getTimestamp()} Progress: ${normalizedPercent}%\n`);
        })
        .on('error', (err: Error) => {
...
export async function mergeVideosWithTransition(
  ...
  try {
    ...
    logSectionHeader(`Merging ${videoFiles.length} videos with ${transitionDuration}s transitions`);
    ...
    const concatVideo = `${videoLabels.join('')}concat=n=${videoFiles.length}:v=1:a=0,format=yuv420p[vout]`;
    const concatAudio = `${audioLabels.join('')}concat=n=${videoFiles.length}:v=0:a=1[aout]`;
