...
async function updateVideoStatus(
  ...
  try {
    ...
    if (error) {
      ...
      logInfo(`Updated video ${videoId} status to ${status}${videoURL ? ' with URL' : ''}`);
    }
  } catch (error) {
...
export async function processVideo(videoId: string): Promise<string | null> {
  try {
    ...
    if (videoError || !video) {
      logError(`Failed to fetch video ${videoId}: ${videoError?.message || "Video not found"}`);
    ...
    if (segmentsError || !segments || segments.length === 0) {
      logError(`Failed to fetch segments for story ${video.storyId}: ${segmentsError?.message || "No segments found"}`);
      await updateVideoStatus(videoId, "failed");
      // Early return so worker can move on to the next item
      return null;
    }
    
    logInfo(`Found ${segments.length} segments for video ${videoId} (story ${video.storyId})`);
    ...
    try {
      for (const segment of segments) {
        try {
          logSectionHeader(`PROCESSING SEGMENT ${segment.id} (index: ${segment.sortedIndex})`);
          ...
          if (videoURL) {
            ...
            logInfo(`Updating segment ${segment.id} with videoURL and completed status`);
            ...
            if (segmentUpdateError) {
              logError(`Failed to update segment ${segment.id} videoURL`, segmentUpdateError);
          ...
          try {
            ...
            logError(`Failed to update segment ${segment.id} status to failed`, updateError);
          }
        }
      }
      ...
      if (hasFailedSegments) {
        logWarning(`Some segments failed during processing for video ${videoId}`);
        await updateVideoStatus(videoId, "failed");
        // Early return so worker can move on to the next item
        return null;
      }
    ...
    logSectionHeader(`MERGING ${processedSegments.length} SEGMENT VIDEOS FOR VIDEO ${videoId}`);
    ...
    try {
      ...
      const finalVideoDir = path.join(config.paths.tempDir, config.paths.processingDir, `${videoId}`);
      ...
      for (const segment of processedSegments) {
        ...
        const segmentVideoPath = path.join(finalVideoDir, `segment_${segment.id}.mp4`);
        const downloaded = await downloadFile(segment.videoURL, segmentVideoPath);
      ...
      logSuccess(`Successfully downloaded ${localVideoFiles.length} segment videos`);
      ...
      try {
        ...
        const fallbackDir = path.join(config.paths.tempDir, config.paths.processingDir, `${videoId}_fallback`);
        fs.mkdirSync(fallbackDir, { recursive: true });
        tempDirectories.push(fallbackDir);
        
        // Download the fallback video only to temp directory
        const fallbackVideoPath = path.join(fallbackDir, `segment_fallback.mp4`);
        ...
        if (downloaded) {
          ...
          logInfo(`Uploading fallback video to dedicated storage folder as ${finalizedStorageFileName}`);
          await uploadFile(fallbackVideoPath, finalizedStorageFileName);
          logSuccess(`Successfully uploaded fallback video to storage`);
        } else {
          logWarning(`Could not download fallback video, but will still update database with URL`);
        }
        ...
        logWarning(`Fallback: Updated video ${videoId} with first segment URL but marked as failed`);
      ...
      try {
        ...
        logError(`Failed to update video ${videoId} status to failed`, updateError);
      }
